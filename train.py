...
def parse_args():
    ...
    parser.add_argument('-c', '--ckpt', type=str, default='new',
                        help='Name of checkpoint to save to. Defaults to timestamp.')
    parser.add_argument('-e', '--epoch', type=int, default=100)
    parser.add_argument('-b', '--batch_size', type=int, default=8192)
...
def train() -> None:
    ...
    all_params = list(model_coarse.parameters()) + \
                 list(model_fine.parameters()) + \
                 list(deformer.parameters())
    ...
    for e in range(args.epoch):
        ...
        if e % args.test_every == 0:
            with torch.no_grad():
                ...
                torch.save(deformer.state_dict(), f"checkpoints/{args.ckpt}/deformer/{e}.pth")
    ...
    torch.save(deformer.state_dict(), f"checkpoints/{args.ckpt}/deformer/final.pth")
